...
class ControllerExtensionModuleTfFilter extends Controller
{
    ...
    public function index($setting)
    {
       ...
       // $data['hide_zero_filter'] = $this->info['hide_zero_filter']; 13-09-2024
       $data['hide_zero_filter'] = isset($this->info['hide_zero_filter']) ? $this->info['hide_zero_filter'] : null;
        ...
        if (isset($this->info['filter']['availability']['status'])) {
            if (isset($this->info['filter']['availability']['stock_status']) && $this->info['filter']['availability']['stock_status']) {
                $url .= '&_f_stock_status';
            } else {
                $url .= '&_f_availability';
            }
    ...
    {
        // if ($this->info['filter']['availability']['stock_status']) { // 13-09-2024
    ...
    private function loadData()
    {
        ...
        // } // 13-09-2024
        if (
            ($this->info && isset($this->info['filter']['availability']['status']) && isset($this->info['filter']['availability']['stock_status'])) || 
            isset($this->request->get['_f_stock_status']) || isset($filter_param['filter_stock_status'])
        ) {
            $additional_field[] = 'p.stock_status_id';
            $additional_field[] = 'p.quantity';
        }
