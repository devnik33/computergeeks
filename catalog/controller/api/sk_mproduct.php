<?php
class ControllerApiSkMproduct extends Controller
{
		...
		foreach ($data['products'] as $product) {
			if (isset($product['barcode']) && strlen($product['barcode']) > 0 && isset($product['barcodeonopencart_field'])) {
				...
				if ($item->num_rows == 1) {
					...
					array_push($updated, $pid . ' __-__ ' . $product['category_description_name']);
				} else {
					//echo $item->num_rows.',';	
					//echo $product['barcode'];
					$pid = $this->addSp($product);
					array_push($new, $pid . ' - ' . $product['category_description_name']);
				}
			} else {
			}
		}
