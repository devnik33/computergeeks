<?php
class ModelCatalogTolisProduct extends Model {
	...
	public function getProductByCode($code, $field) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p WHERE ".$field." = '".$code."'");
	...
	public function updateExisting($product,$priceIds)
	{
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product_price 
				SET price='" . $this->db->escape($product['wholesale_price']) . "'
				WHERE product_id='".$p->row['product_id']."' and customer_group_id='".$this->db->escape($priceIds['Wholesale'])."'");

		$this->db->query("UPDATE " . DB_PREFIX . "product_price 
				SET price='" . $this->db->escape($product['retail_price']) . "'
				WHERE product_id='".$p->row['product_id']."' and customer_group_id='".$this->db->escape($priceIds['Retail'])."'");
